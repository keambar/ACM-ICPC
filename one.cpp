...
//计算两点距离，算出用时。参数为两点经纬。
int usetime(position a,position b){
...
struct rst{
	...
	position pos_ret;
	int make_order_time;
	rst(int ret_id,double lng,double lat,int make_order_time):ret_id(ret_id),make_order_time(make_order_time){
		pos_ret=make_pair(lng,lat);
	};
};
//订单数据
struct order{
	...
	order(int order_id,int rst_id,double lng,double lat,string created,string promised):order_id(order_id),rst_id(rst_id){
		...
		promised_at=gettime(promised);
		created_at=gettime(created);
		reached_at=0;
	};
};
//骑手数据
...
void readinput()
{
	...
	for(int i=0;i<n;++i){
		...
		int id,usetime;
		double lng,lat;
		sscanf(s,"[%d,%lf,%lf,%d]",&id,&lng,&lat,&usetime);
		//printf("[%d,%lf,%lf,%d]",id,lng,lat,usetime);
		in_ret[id]=(rst(id,lng,lat,usetime));
	}
	...
	for(int i=0;i<m;++i){
		...
		tmp.makeup_at=tmp.created_at+in_ret
		in_order[order_id]=tmp;
		getordertime[order_id]=tmp.created_at+make_order[rst_id];
	...
	for(int i=0;i<k;++i){
		...
		in_courier.push_back(tmp);
		maxload[courier_id]=max_load;
	}
}
...
int check()
{
	...
	while(gets(s)>0){
		...
		if(usedcourier.find(courier_id)==usedcourier.end()){
			...
			//3m/s的速度不满足经纬移动
			if(lasttime+usetime(lastpos,nowpos)>now) return -1;
			...
			if(type==1){
				...
				if(orderfinish.find(order_id)==orderfinish.end()){
					++pick;
					//骑手超过负载。
					if(pick>maxload[courier_id]){
						return -1;
					}
					//取餐标记
					orderfinish[order_id]=0;
					//取餐时间是早于出餐时间
					if(now<getordertime[order_id]){
						return -1;
					}	
				}
				else {
