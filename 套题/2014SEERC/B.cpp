...
#define ll long long
#define MAXN 1000009
int sa[MAXN], pos[MAXN], Rank[MAXN], r[MAXN], height[MAXN],tank[MAXN];
int d, n, k;
string s;
...
{
	int i, j, k = 0;
	for (i = 1; i <= n; i++) tank[sa[i]] = i;
	for (i = 0; i < n; height[tank[i++]] = k)
		for (k ? k-- : 0, j = sa[tank[i] - 1]; r[i + k] == r[j + k]; k++);
	return;
}
